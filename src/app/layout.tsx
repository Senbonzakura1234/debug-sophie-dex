...
const ThemeSwitcher = dynamic(
	() => import("@root/components/layout/client/ThemeSwitcher"),
	{ ssr: false }
);
...
export const metadata: Metadata = {
	...
	keywords: process.env.NEXT_PUBLIC_APP_KEYWORD?.split(","),
	manifest: "/manifest.json",
	metadataBase: new URL(getBaseUrl(true)),
	other: {
		google: "notranslate",
		"og:title": `sophie-dex | ${process.env.NEXT_PUBLIC_APP_DESCRIPTION}`,
		"og:description": process.env.NEXT_PUBLIC_APP_DESCRIPTION || "",
		"og:url": getBaseUrl(true),
	},
	robots: "all",
	themeColor: "#996c254d",
	title: `sophie-dex | ${process.env.NEXT_PUBLIC_APP_DESCRIPTION}`,
	twitter: {
...
export default async function RootLayout({ children }: ChildrenProps) {
	...
	return (
		<html lang="en">
			<body
				className={`${fontAtelier.variable} ${fontComicSansMS.className}`}
			>
				<ContextProvider
					defaultState={{
						theme: daisyUIThemeEnumSchema.parse(
							cookiesList.get("theme")?.value
						),
					}}
				>
					<ThemeWrapper>
